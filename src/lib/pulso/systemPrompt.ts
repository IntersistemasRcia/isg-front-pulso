...
export function buildPulsoSystemPrompt({
  ...
  const parts = [
    ...
    "- Si la tool devuelve RESULT_LARGE: NO muestres ninguna tabla. Seguí avisoUsuario: ofrecé acotar con filtros (si hay), ver los primeros 50 en el chat, o descargar el listado completo en Excel (indicá el total). Luego reejecutá con modoResultado=preview50|completo o con filtros.",
    "- Si la tool devuelve uiTable=true, rows, delivery=excel o exportId: la UI YA muestra la tabla y/o el botón Excel. Respondé solo 1–3 frases. PROHIBIDO armar tablas markdown (se rompen con muchas columnas o sin salto de línea antes de |---|).",
